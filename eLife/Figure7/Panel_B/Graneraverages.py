...
class Graneraverages:
	...
	# Averaging this one: see notes
	# 5 October 2020: implementing a cutoff in how soon a T1 in the same place can happen.
	def statTtensor(self, plotvtk = False, verbose = True):
		...
		u=0
		# keep a track of cells involved in T1 transition in the previous two steps
		# remove the T1 from the list if the same cells appear again
		invkeep=[]
		buf = []
		for i in range(self.start,self.filemax-self.step,self.step):
			# accumulating T1s from this step
			inv=[]
			if i % 50 == 0:
				print("files number {:d} and {:d}.".format(i, i + self.step))
			f1 = self.allfiles[i]
			...
			for j in self.labels:
				...
				if abs(tr)>0:
					# add to the current T1 list
					inv.append(j)
					# if it's not in the previous list
					if not j in invkeep:
						self.NT1[u] +=1
						if tr  < 0:
						...
						print('Threw out T1 at particle ' + str(j) + ' at frame ' + str(i))
			...
			# add current T1s to the list to exclude for the next steps
			invkeep=inv+buf
			# buffer to have an extra one step memory
			buf=inv
			print(invkeep)
			u+=1
...
##################### Continuum plastic and elastic strain functions ###################
	...
	def getU(self, reference = 'default', plot = True):
		...
		if plot:
			...
			plt.plot(self.tvals,self.U[:,0,0],'-r',label='Uxx')
			plt.plot(self.tvals,self.U[:,1,1],'-k',label='Uyy')
			plt.plot(self.tvals,self.U[:,0,1],'-g',label='Uxy')
			plt.xlabel('time')
	...
	def getP(self, plot = True):
		...
		if plot:
			...
			tvals = self.times
			# Plot the different components. Note this is a symmetric tensor
			plt.plot(self.tvals[:(self.Nsnap-1)],self.P[:,0,0],'-r',label='Pxx')
			plt.plot(self.tvals[:(self.Nsnap-1)],self.P[:,1,1],'-k',label='Pyy')
			plt.plot(self.tvals[:(self.Nsnap-1)],self.P[:,0,1],'-g',label='Pxy')
			plt.xlabel('time')
	...
	def getV(self, plot = True):
		...
		if plot:
			plt.figure()
			# Plot the different components. Note this is a symmetric tensor
			plt.plot(self.tvals[:(self.Nsnap-1)],self.V[:,0,0],'-r',label='Vxx')
			plt.plot(self.tvals[:(self.Nsnap-1)],self.V[:,1,1],'-k',label='Vyy')
			plt.plot(self.tvals[:(self.Nsnap-1)],self.V[:,0,1],'-g',label='Vxy')
			plt.xlabel('time')
	...
	def getOmega(self, plot = True):
		...
		if plot:
			plt.figure()
			# Plot the different components. Note this is a symmetric tensor
			plt.plot(self.tvals[:(self.Nsnap-1)],self.Omega[:,0,0],'-r',label='Oxx')
			plt.plot(self.tvals[:(self.Nsnap-1)],self.Omega[:,1,1],'-k',label='Oyy')
			plt.plot(self.tvals[:(self.Nsnap-1)],self.Omega[:,0,1],'-g',label='Oxy')
			plt.xlabel('time')
...
################ Basic statistics ####### limit bins given data issues
	...
	# Extract the junction properties of the central junction and its four shoulders
	def getInnerDynamics(self,usetype="none",verbose=True):
		# use the first active cell as starting point
		# Search again since ideally we want to run this concurrently with the other analysis
		tensor = MTensor(self.allfiles[self.start])
		# Basically: Have the option of giving this a subset of the cells, or a different type to the rest of the analysis
		if usetype != "none":
			useme = usetype
		else:
			useme = self.usetypes
		for fc in tensor.mesh.faces:
			if fc.type in self.usetypes:
				startidx = fc.idx
				break
		myosins = np.zeros((self.Nsnap,10))
		tensions = np.zeros((self.Nsnap,5))
		l0s = np.zeros((self.Nsnap,5))
		lengths = np.zeros((self.Nsnap,5))
		f1id = np.zeros((self.Nsnap,))
		f2id = np.zeros((self.Nsnap,))
		isdata = []
		# Checking for the time for first and second T1 (as in 'i' label)
		# As measured by the change of the id of central junction
		firstT1 = -1
		secondT1 = -1
		for i in range(self.start,self.filemax,self.step):
			if verbose:
			...
			# get values
			bst = BasicStatistics(self.allfiles[i])
			success,f1id0,f2id0,myosins0,tensions0,l0s0,lengths0 = bst.getEdgeDetails(startidx,usetype,verbose=verbose)
			isdata.append(success)
			f1id[i] = f1id0
			f2id[i] = f2id0
			# looking for second T1: First needs to have happened already
			# and we don't care what it becomes, just that there is a further change
			if (f1id[i-1]>0) and (firstT1 >0):
				if ((f1id[i] != f1id[i-1]) or (f2id[i] != f2id[i-1])):
					secondT1 = i
					print('Found second T1 at time ' + str(i))
			# looking for first T1: Central edge changes to another legal value
			if (f1id[i-1]>0) and (firstT1 == -1):
				if (f1id[i]>0) and ((f1id[i] != f1id[i-1]) or (f2id[i] != f2id[i-1])):
					# Further empirical constraints: There are some very odd things at high activity 
					# where the central junction twists out of plane and then jumps discontinuously 
					if lengths[i-1,0]<0.5:
						firstT1 = i
						print('Found first T1 at time ' + str(i))
			# for the topology where everything checked out (and only then), record the data
			if success:
				myosins[i,:]=myosins0
				tensions[i,:]=tensions0
				l0s[i,:]=l0s0
				lengths[i,:]=lengths0
				
		return isdata,firstT1,secondT1,myosins,tensions,l0s,lengths
