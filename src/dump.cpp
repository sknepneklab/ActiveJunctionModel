...
namespace AJM
{
  void Dump::dump_cells(const string& vtk_file, bool binary_output, bool draw_periodic)
  {
    ...
    vtkSmartPointer<vtkDoubleArray> myo =  vtkSmartPointer<vtkDoubleArray>::New();
    vtkSmartPointer<vtkDoubleArray> myo_alpha =  vtkSmartPointer<vtkDoubleArray>::New();
    ...
    myo->SetName("Myosin");    
    myo->SetNumberOfComponents(1);
    myo_alpha->SetName("Myosin Alpha");    
    myo_alpha->SetNumberOfComponents(1);
      ...
      if (!fh->erased)
      {
        ...
        if (!(fh->outer || omit))
        {
          ...
          do
          {
            ...
            face_myo += he->data().myo;
            face_myo_alpha += he->data().myo*(1+alpha*le);
            VertexHandle<Property> vh = he->from();
            
            if (vh->coordination > 3) // if collapsed junction vertex
              face_myo_alpha += _sys.get_myostore(vh).get_myo(he->face()->id);
            he = he->next();
          } while (he != first);
          ...
          myo->InsertNextValue(face_myo);
          myo_alpha->InsertNextValue(face_myo_alpha);
    ...
    polydata->GetCellData()->AddArray(myo_alpha);
      ...
      if (!fh->erased)
      {
        ...
        do
        {
          ...
          {		      
            ...
            myo->InsertNextValue(he->data().myo); // this is now the HALF-EDGE myosin		
  ...
  void Dump::dump_spokes(const string& spokes_file, bool binary_output)
  {
    vtkSmartPointer<vtkPolyData> polydata =  vtkSmartPointer<vtkPolyData>::New();
    vtkSmartPointer<vtkPoints> points = vtkSmartPointer<vtkPoints>::New();
    vtkSmartPointer<vtkCellArray> lines =  vtkSmartPointer<vtkCellArray>::New();

    vtkSmartPointer<vtkLine> spoke =  vtkSmartPointer<vtkLine>::New();
    vtkSmartPointer<vtkDoubleArray> myo =  vtkSmartPointer<vtkDoubleArray>::New();
    vtkSmartPointer<vtkDoubleArray> l0 =  vtkSmartPointer<vtkDoubleArray>::New();
    ...
    for (FaceHandle<Property> fh = _sys.mesh().faces().begin(); fh != _sys.mesh().faces().end(); fh++)
    {
      if (!fh->outer)
      {
        ...
        HEHandle<Property> first = he;
        Vec fc = _sys.mesh().get_face_centre(fh); // face centre coordinates
        points->InsertNextPoint(fc.x, fc.y, 0.0);
        int vert_id = centre_id + 1;
        do
        {
          VertexHandle<Property> vh = he->from();
          Vec r_shift = fc + _sfc*(vh->r - fc); 
          points->InsertNextPoint(r_shift.x, r_shift.y, 0.0);
          spoke->GetPointIds()->SetId(0, centre_id);
          spoke->GetPointIds()->SetId(1, vert_id);
          lines->InsertNextCell(spoke);
          myo->InsertNextValue(fh->data().spoke[vh->id].myo);
          l0->InsertNextValue(fh->data().spoke[vh->id].l0);
          vert_id++;
          he = he->next();
        } while (he !=first);
        centre_id = vert_id;
    ...
    // Write the file
    vtkSmartPointer<vtkXMLPolyDataWriter> writer =  vtkSmartPointer<vtkXMLPolyDataWriter>::New();
    writer->SetFileName(spokes_file.c_str());
    #if VTK_MAJOR_VERSION <= 5
      writer->SetInput(polydata);
    #else
      writer->SetInputData(polydata);
  ...
  // Outputs myosin values on each half-edge into a text file
  void Dump::dump_myosin(const string& save_file)
  {
    ofstream mymyosin;
    mymyosin.open(save_file);
    for ( HEHandle<Property>  he = _sys.mesh().halfedges().begin(); he != _sys.mesh().halfedges().end(); he++)
    {
      // do not output edge myosin
      if (!he->face()->outer) 
        mymyosin << he->data().myo << endl;
    }
  ...
  void export_Dump(py::module& m)
  {
    py::class_<Dump>(m, "Dump")
        ...
        .def("dump_spokes", &Dump::dump_spokes, py::arg("vtk_file"), py::arg("binary_output") = false)
        .def("dump_mesh", &Dump::dump_mesh, py::arg("mesh_file"), py::arg("copy_params") = false)
        .def("dump_area", &Dump::dump_area)
        .def("dump_myosin", &Dump::dump_myosin)
