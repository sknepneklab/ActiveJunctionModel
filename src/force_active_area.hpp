...
/*!
 * \file force_active_area.hpp
 * \author Rastko Sknepnek, sknepnek@gmail.com
 * \date 20-May-2019
 * \brief ForceActiveArea class 
 */ 

#ifndef __FORCE_ACTIVE_AREA_HPP__
#define __FORCE_ACTIVE_AREA_HPP__
...
namespace AJM
{
  ...
  // Force on a vertex
  class ForceActiveArea : public Force
  {
    public:
      ...
      ForceActiveArea(System& sys) : Force(sys) 
                                    { 
                                      _beta.resize(_sys.cell_types().size(), 0.0);
                                    }
      virtual ~ForceActiveArea() { }
      ...
      {
        //throw runtime_error("Stress calculation has not been implemented for active area force.");
      }
      ...
      // Energy of active are is currently not defined, so we return 0
      ...
      void set_params(const string& cell_type, const params_type& params) override
      {
        ...
        try 
        {
          if (_sys.cell_types().find(cell_type) == _sys.cell_types().end())
            throw runtime_error("Fore active area: Cell type " + cell_type + " is not defined.");
        ...
        {
          cerr << "Problem with setting active area force paramters. Exception: " << e.what() << '\n';
          throw;
        }
      }
      ...
      void set_relative_paramters(const string& cell_type, const multi_params_type& params) override
      {
        ...
        {
          cerr << "Problem with setting relative paramters for active area force. Exception: " << e.what() << '\n';
          throw;
        }  
      ...
      { 
        ...
        {
        _use_cell_type = true;
        cout << "Warning! Setting use_cell_type flag in active area will override all parameters read from the input JSON file (if the read_params flag in the read_input function was set to True)." << endl;
        }
        else
