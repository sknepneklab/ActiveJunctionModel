...
namespace AJM
{
  ...
  void System::read_vertices(const string& vert_file)
  {
    _vert_types["regular"]=0; // need to create a type for regular vertices
    ifstream input(vert_file.c_str());
    if (input.fail())
    {
      cerr << "File " << vert_file << " does not exist" << endl;
      throw runtime_error("File "+vert_file+" does not exist.");
    }
    for (string line; getline(input, line); )
    {
      ...
      {
        vector<string> ln = split_string(line);
        int id = stoi(ln[0]);
        double x = stod(ln[1]);
        double y = stod(ln[2]);
        _mesh.add_vertex(Vertex<Property>(id,Vec(x,y,_mesh.box())));
        Vertex<Property>& v = _mesh.vertices().back();
        v.data().vert_type = 0; //default vertex type 
        v.data().constraint = "none"; //vertex has no constraints by default
      }
    }
    cout << "Finished reading vertices." << endl;
  }

  void System::read_faces(const string& face_file)
  {
    ifstream input(face_file.c_str());
    if (input.fail())
    {
      cerr << "File " << face_file << " does not exist" << endl;
      throw runtime_error("File "+face_file+" does not exist.");
    }
    for (string line; getline(input, line); )
    { 
      line = trim(line);
      if (line.size() > 0 && line[0] != '#')
      {
        ...
        _mesh.add_face(verts);
        verts.clear();
      }
    }
    cout << "Finished reading cells." << endl;
  }

  void System::read_boundary(const string& boundary_file)
  {
    ifstream input(boundary_file.c_str());
    if (input.fail())
    {
      cerr << "File " << boundary_file << " does not exist" << endl;
      throw runtime_error("File "+boundary_file+" does not exist.");
    }
    for (string line; getline(input, line); )
    { 
      line = trim(line);
      if (line.size() > 0 && line[0] != '#')
      {
        vector<string> ln = split_string(line);
        int id = stoi(ln[0]);
        VertexHandle<Property> vh = find_if(_mesh.vertices().begin(), _mesh.vertices().end(), [id](const Vertex<Property>& v)->bool { return (v.id == id); });
        if (vh != _mesh.vertices().end()) vh->boundary = true;
	// find a way to make constraints on the boundary vertices 
      }
    }
    cout << "Finished reading boundary." << endl;
  }

  void System::read_face_types(const string& face_type_file)
  {
    int curr_cell_type=0;        
    ifstream input(face_type_file.c_str());
    if (input.fail())
    {
      cerr << "File " << face_type_file << " does not exist" << endl;
      throw runtime_error("File "+face_type_file+" does not exist.");
    }
    FaceHandle<Property> fh = _mesh.faces().begin();
    for (string line; getline(input, line); )
    { 
      line = trim(line);
      if (line.size() > 0 && line[0] != '#')
      {
        ...
        //int tp = stoi(ln[0]);	
        string tp = ln[0];	
        if (_cell_types.find(tp) == _cell_types.end())
          _cell_types[tp] = curr_cell_type++; // increment curr_cell_type
        fh->data().face_type = _cell_types[tp];		
        fh = std::next(fh);
      }
    }
    
    for (EdgeHandle<Property> eh = _mesh.edges().begin(); eh != _mesh.edges().end(); eh++)
      eh->data().l0 = 1.0;    //set l0 to default value 
  }
  ...
  void System::set_parameters(double myo_max)
  {
    RNG rng(0);
    for (Edge<Property>& e : _mesh.edges())
    {
      double myo = myo_max; //*rng.drnd();
      e.data().myo = myo;
      HEHandle<Property> he = e.he();
      he->data().myo = myo;
      he->data().myo_next = 0.0;
      he->pair()->data().myo = myo;
      he->pair()->data().myo_next = 0.0;
    }
  }

  void System::set_cell_myosin(int faceid, double myo)
  {
    for (Face<Property>& f : _mesh.faces())
      if (f.id == faceid)
      {
        f.data().cell_myo = myo;
        return; 
  ...
  void System::set_cell_myosin_internal(FaceHandle<Property> fh, double myo)
  {
    fh->data().cell_myo = myo;
    return; 
  }
  ...
  void export_System(py::module& m)
  {
    py::class_<System>(m, "System")
      .def(py::init<MyMesh&>())
      .def("read_vertices", &System::read_vertices)
      .def("read_cells", &System::read_faces)
      .def("read_boundary", &System::read_boundary)
      .def("read_face_types", &System::read_face_types)
