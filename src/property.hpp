...
namespace AJM
{
  ...
  struct T1_stats
  {
    ...
    double myo1, myo2;  // collapse myosin
  ...
  struct Spoke
  {
    double myo = 0.0;   // myosin in the spoke
    double l0 = 0.0;    // native (rest) length of the spoke
  };
  ...
  {  
    struct HEProperty : public BaseProperty::HEProperty
    {
      ...
      double l0_collapse;   // myosin value before the collapse
      double myo_next;    // keeping the myosin concentration of the next he, in case of collapse, otherwise reset
    ...
    struct FaceProperty : public BaseProperty::FaceProperty
    {
      ...
      double cell_myo;      // total amount of myosin a cell can have
      double D;            // myosin diffusion constant for a given cell
      ...
      double k_spoke;     // stiffness constant each spoke
      ...
      map<int, Spoke> spoke;    // spoke connecting centre of the cell with each vertex, indexed by vertex id
      ...
      vector<double> myo; // myosin on each junction
      double active_myo;    // total amount of active myosin in a cell
      void add_spoke(int vid, const Spoke &s) { spoke[vid] = s; }
