      ...
      if (!f1.outer)
      {
        ...
        do
        {
          ...
          if ( f2->id == id2  && !f2->outer  ){
            ...
            double myo = he->data().myo + he->pair()->data().myo;
...
// decided to not make another populate here since this uses the already known half-edges
// This will the polarization tensor in the end
void Measurement::compute_myotexture()
{
  double mx2 = 0.0, mxy = 0.0, my2 = 0.0;
  for(FaceHandle<Property> fh = _mesh.faces().begin(); fh != _mesh.faces().end(); fh++)
  {
    if (!fh->outer)
    {
      ...
      do
      {
        ...
        Vec l = v_to->r - v_from->r;
        // myosin prefactor is the mean of both half-edges
        double myofact = 0.5*(he->data().myo + he->pair()->data().myo);
        // put the texture of the edges in here, multipled by the myosin prefactor
        mx2 = myofact*l.x*l.x; mxy = myofact*l.x*l.y; my2 = myofact*l.y*l.y;
        // save per edge in same kind of texture structure
        _sites[id].myom.push_back(Texture(mx2,mxy,my2));
        he = he->next();
      } while (he != first);
    }
  }
...
void Measurement::dump_myotexture(const string& filename)
{
  ...
  for (Site& s : _sites)
  {
    ...
    for (int i = 0; i < s.myom.size(); i++)
    {
      XX += s.myom[i].xx;  XY += s.myom[i].xy;  YY += s.myom[i].yy; 
    }
    int Ntot = s.myom.size();
    XX /= Ntot;  XY /= Ntot;  YY /= Ntot;
    out << s.id << "  " << s.r.x << "  " << s.r.y << "  " << XX << "  " << XY << "  " << YY << std::endl;
  }
  out.close();
}
    ...
    {          
      ...
      { // if the two faces are neighbors find common junction and dump it 
        ...
        if (f->id == id2 && !f->outer ){
          Vec l = he->to()->r - he->from()->r;
          double myo = he->data().myo + he->pair()->data().myo ;
          out << _sys.time_step() << "  " << l.x << "  " << l.y << "  " << myo << endl;
        }
        he = he->next();
      } while (he != first);
    }
...
void export_Measurement(py::module& m)
{
  py::class_<Measurement>(m, "Measurement")
    ...
    .def(py::init<MyMesh&,System&>())
        ...
        .def("myotexture", &Measurement::compute_myotexture)
        .def("dump_myotexture", &Measurement::dump_myotexture);
}
